...
发现省份的翻译有问题，state应该改成省，zip改成邮编
搜索源术语包含state，翻译值包含“状态”，已翻译字段为“ir.ui.view,arch_db”的第五个
搜索翻译值包含ZIP，已翻译字段为“ir.ui.view,arch_db”的第三个
搜索翻译值包含www.odoo.com

计量单位翻译值的修改，在计量单位页面，点击地球图标直接修改翻译值
...
导入产品类别product.category.xls
导入产品product.xls
导入供应商/客户信息customer.xls,在tree视图可以导入
...
把生产成本的类型从“业务成本”改到“流动负债”，通过修改l10n_cn_yto_chart_data.xml并不能生效，把其文件顶部的 <data noupdate >里的noupdate删除，等重新布置系统时再观察。实际上在“外部标识”里面有一项“不可更新”，据说把这个取消了就能更新。最后在会计模块里的科目表里面手动修改42条记录，

把自制件（含主机）的库存计价方法从“永续（自动）”改成“定期（手工）”后，就可以在产品页面直接填价格，不用填写一个“conter-part account”的会计科目（不知道干什么的），重新导入了product.category.xls
在库存价格调整的菜单放到财务的管理里面，只有财务的顾问才能访问到。

存档07Show02.backup

存档07Show03.backup一笔入库，未挂账

恢复到07Show02.backup，在“设置”-》“通用设置”里面有“新用户访问权”一项，点击进入，设置权限，一般员工即可，配置完账户后再增加具体权限，然后登陆其账户测试主要权限，最后交付用户。
又把研发的两个表导入，可以准备演示了
存档07Show04.backup
